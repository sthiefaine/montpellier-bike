...
import { getStartOfMonth, getEndOfMonth, toParisTime, getStartOfDay } from "./dateHelpers";
...
export async function getDailyStatsForYear(counterId: string) {
    ...
    Prisma.sql`
      WITH dates AS (
        SELECT generate_series(
          date_trunc('day', ${getStartOfMonth(startOfYear)}::timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'Europe/Paris'),
          date_trunc('day', ${getEndOfMonth(endOfYear)}::timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'Europe/Paris'),
          interval '1 day'
        )::date as day
      ),
      daily_stats AS (
  ...
  const today = getStartOfDay(toParisTime(new Date()));
